...
## 简介
[src/services/CategoryService.ts](file://src/services/CategoryService.ts) 编排 URL 分类：缓存优先，未命中则调用 AI 模型，并将结果写回缓存。分类结果属于 11 类 `UrlCategory` 之一。

## getCategory
```ts
async function getCategory(url: string, html: string, option?: Option): Promise<...>
```
主入口。流程：`extractDomain` → `getCategoryFromDB`（命中即返回）→ 未命中则 `loadOption` + `initClient` + `categorifyModel` 调用模型 → 校验类别合法 → `urlCategoryDB.put` 写回。
...
## 分类提示词与输出契约
提示词内置 11 个类别及其判定说明，约束模型输出为严格两行文本：第一行为归一化 domain，第二行为类别标识。`categorifyModel` 会校验第二行必须属于 [UrlCategory](file://src/models/types.ts) 枚举，否则视为无效。
