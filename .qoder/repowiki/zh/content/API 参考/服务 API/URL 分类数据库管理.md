...
[src/services/UrlCategoryDataBaseManager.ts](file://src/services/UrlCategoryDataBaseManager.ts) 以单例 `urlCategoryDB`
管理 URL 分类缓存，基于 `idb` 封装 IndexedDB。
...
`lookup` 从二级域名（`MIN_LEVEL = 2`）开始逐级向上匹配到最多 5 级（`MAX_LEVEL = 5`），命中第一个即返回。例如
`a.b.c.example.com` 依次尝试：
