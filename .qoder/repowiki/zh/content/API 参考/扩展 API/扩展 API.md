...
## 简介
扩展 API 指 BrainRest 基于 Chrome 扩展平台使用的运行时机制，包括长连接与消息传递、存储访问、事件监听注册与生命周期。它们围绕 Manifest V3 的 service worker 模型组织。

## 扩展清单概览
[src/manifest.ts](file://src/manifest.ts) 以默认导出对象加 `satisfies ManifestV3Export`（来自 `@crxjs/vite-plugin`）声明清单，关键字段：
