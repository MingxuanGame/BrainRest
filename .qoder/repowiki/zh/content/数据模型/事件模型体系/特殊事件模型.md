...
由 [src/background/WindowFocusListener.ts](file://src/background/WindowFocusListener.ts) 产生：`chrome.windows.onFocusChanged` 回调中，窗口 ID 为 `WINDOW_ID_NONE` 时生成 `blur`，否则生成 `focus`，`url` 为空串并 `queue.push`。窗口长时间失焦（≥ `WINDOW_BLUR_MS = 30000`）会提升休息权重。
...
由 [src/content/DomListener.ts](file://src/content/DomListener.ts) 监听 `fullscreenchange` 产生，`active` 表示是否进入全屏。视频全屏场景会通过 `dispatcher.setVideoFullscreen` 提升休息权重（`videoFullscreen = 30`）。
