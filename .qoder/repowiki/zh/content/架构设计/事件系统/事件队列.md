...
## 简介
事件队列是后台的中央事件缓存，实现为一个只保留最近 5 秒事件的“滑动窗口队列”（`SlidingWindowQueue`）。它以单例 `queue` 形式导出，被所有事件生产者（Port 接收、标签页/窗口监听器）写入，被疲劳指数引擎读取。它**不做持久化**——历史事件到期即丢弃。
...
## 与疲劳引擎的关系
`RuleEventDispatcher` 每秒 `tick` 一次，调用 `queue.getEvents()` 拿到当前 5 秒窗口，据此计算标签页切换、鼠标熵、眼手延迟、事件频率等指标。窗口长度（5s）与引擎节拍（1s）共同决定了疲劳指数的时间灵敏度。
