...
所有被采集的事件都实现自统一的 `Event` 基接口，并通过字符串字面量 `type` 作为判别式（discriminated union）。这套模型定义在
`src/models/events` 目录下，被内容脚本、后台监听器与疲劳引擎共享。
...
基接口的 `type: string` 在各子接口中被收窄为字面量（如 `'click'`、`'tab_changed'`），配合 `createEvent<T>` 工厂即可在消费侧用
`switch (event.type)` 做类型收窄。`createEvent` 自动填充 `processed: 0` 与 `timestamp: Date.now()`。
