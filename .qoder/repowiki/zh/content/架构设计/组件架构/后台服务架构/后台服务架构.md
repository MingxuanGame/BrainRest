...
后台运行在 MV3 的 module 类型 service worker 中，是扩展的处理中枢：既维护事件队列与认知负荷引擎，又充当分类请求的执行者。
...
2. `engine.start()` 启动每 30 秒认知负荷计算。
3. 注册 `onConnect`：过滤 `event-stream` Port，按类型分流事件（`page_complexity` 转发引擎、`tab_activated` 写入 TabEventBuffer、所有事件入队并转发引擎）。
...
graph TB
subgraph 认知负荷流水线
Q["queue"] --> H["helper 分析器"]
H --> E["engine.tick 每30s"]
E --> R["BRIResult (含 triggerPath)"]
end
subgraph 分类服务
