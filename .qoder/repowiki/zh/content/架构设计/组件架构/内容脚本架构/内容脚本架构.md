...
内容脚本运行在每个匹配页面的隔离环境中，负责采集页面内交互并触发页面分类。它由三个纯副作用模块组成，通过 `index.ts`
一次性导入完成注册。

## 注入方式

`manifest.ts` 声明 `content_scripts` 匹配 `<all_urls>`，入口为 `src/content/index.ts`；`vite.config.ts` 将其列入
`standaloneFiles` 独立打包。
...
`index.ts` 仅有三行 `import`：导入 `EventChannel`（建立 Port）、`DomListener`（注册监听）、`AutoCategorizer`（挂载 load
回调）。三个模块均在导入时通过顶层副作用完成初始化，没有显式的 init 函数。
