...
BrainRest 的数据流围绕“事件”与“分类”两类数据展开。事件从页面/浏览器采集，进入 5 秒滑动窗口，被认知负荷引擎每 30 秒消费一次；分类数据由
AI 生成后写入 IndexedDB，供反查复用。本文描述真实的数据流转，而非通用的“采集-清洗-入库-同步”管线。
...
subgraph 处理
Q["EventQueue 5s 窗口"]
D["CognitiveLoadEngine 每 30s 计算"]
end
DL --> EC --> Q
TL --> Q
Q --> D --> R["BRIResult（含 triggerPath）"]
...
3. **处理**：`CognitiveLoadEngine` 每 30 秒读取窗口与页面复杂度，计算脑休息指数 BRI。见[数据处理引擎](数据处理引擎.md)。
...
| 消费者 | 认知负荷引擎          | 分类反查（CategoryService 缓存）     |
