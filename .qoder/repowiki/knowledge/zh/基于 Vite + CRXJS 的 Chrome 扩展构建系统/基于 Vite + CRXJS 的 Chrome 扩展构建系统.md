...
本项目采用 **Vite + TypeScript + React** 作为前端工程化基础，通过 `@crxjs/vite-plugin` 将标准 Vite 应用直接打包为符合 Manifest V3 规范的 Chrome 扩展。整个构建流程完全由 npm scripts 驱动，无 Makefile、Dockerfile 或 CI 配置文件。
