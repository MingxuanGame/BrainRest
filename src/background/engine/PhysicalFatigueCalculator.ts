 ...
 */
export function calculatePhysicalFatigue(restState: RestState): {
    clPhy: number;
    ...
    const L = delayMs === null ? 0 : clamp((delayMs / MAX_EYE_HAND_DELAY_MS) * 100, 0, 100);
    ...
    const clPhy = clamp((weightedSum / 100) * (1 - R_rest / 100) * 100, 0, 100);
...
function computeRestWeight(state: RestState): number {
    ...
    if (!state.isFocused && state.lastBlurAt !== null && now - state.lastBlurAt > WINDOW_BLUR_MS) {
