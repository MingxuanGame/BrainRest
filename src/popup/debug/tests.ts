        ...
        },
        {
            ...
            run: async () => {
                ...
                if (Math.abs(actual - expected) > 2) {
                    throw new Error(`频率 ${actual.toFixed(2)}/s 与队列快照推算值 ${expected.toFixed(2)}/s 偏差过大`);
                }
        ...
        },
        {
            ...
            run: async () => {
                ...
                if (!eventStats.portAlive) {
                    throw new Error("event-stream 端口已断开（service worker 可能被回收后未重连，刷新页面可恢复）");
                }
    ...
    subTests: [
        {
            ...
            run: async () => {
                ...
                const response = (await chrome.runtime.sendMessage(request)) as CategorizeResponse | undefined;
                if (!response) {
                    throw new Error("service worker 无应答");
                }
                if (!response.ok) {
                    throw new Error(`分类失败: ${response.error ?? "未知错误"}（未命中缓存时需配置 apiKey）`);
                }
