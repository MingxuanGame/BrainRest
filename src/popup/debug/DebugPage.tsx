...
function SubTestRow({test, state, disabled, onRun}: {
    test: SubTest;
    state: TestState;
    disabled: boolean;
    onRun: () => void;
...
function GroupCard({group, results, expanded, busy, onToggle, onRunSub, onRunGroup}: {
    group: TestGroup;
    ...
    const passCount = group.subTests
        .filter((t) => results[t.id]?.status === "pass").length;
    ...
    return (
        <div style={{width: 440, maxHeight: 580, overflowY: "auto", padding: 12, fontSize: 12}}>
            <div style={{display: "flex", alignItems: "center", gap: 8, marginBottom: 8}}>
                ...
                <button onClick={() => void runAll()} disabled={runningAll} style={{marginLeft: "auto"}}>
                    {runningAll ? "运行中…" : "运行全部"}
                </button>
            </div>
